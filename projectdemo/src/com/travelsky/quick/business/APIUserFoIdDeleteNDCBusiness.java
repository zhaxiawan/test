 ...
 * @version 0.1
 * 类型说明:
 *	  		USER_DELETE_FOID 证件号
 */
@Service("LCC_USERFOIDDELETE_SERVICE")
public class APIUserFoIdDeleteNDCBusiness extends AbstractService<ApiContext>{

	/**
	 * 
	 */
	private static final long serialVersionUID = -1465398137293482828L;
	private static final Logger LOGGER = LoggerFactory.getLogger(APIUserFoIdDeleteNDCBusiness.class);
	@Override
	...
	public CommandRet getResponseBean() {
		...
		//会员信息删除_证件号
		return memberManager.delID(input, context);	
	}
	/**
	...
	public static String getID(Modify modify)
		...
		String id = "";
		//会员信息删除_证件号
		FOID[] info =modify.getLCCUserInfo().getFOIDArray();
		 if(null != info && info.length>0){
			 id = info[0].getFOIDKey();
