 ...
 * @version 0.1
 * 类型说明:
 * 			USER_DELETE_CONTACTMETHOD 联系方式
 */
@Service("LCC_USERCONTACTMETHODDELETE_SERVICE")
public class APIUserContactMethodDeleteNDCBusiness extends AbstractService<ApiContext>{

	/**
	 * 
	 */
	private static final long serialVersionUID = -1465398137293482828L;
	private static final Logger LOGGER = LoggerFactory.getLogger(APIUserContactMethodDeleteNDCBusiness.class);
	@Override
	...
	public void transInputXmlToRequestBean() throws APIException, Exception {
		...
		//删除信息类型
		String  actionType = modify.getActionType();
	...
	public CommandRet getResponseBean() {
		...
		if ("USER_DELETE_CONTACTMETHOD_EMAIL".equals(actionType)) {
			return memberManager.delContactEmail(input, context);
		}else{
			return memberManager.delContact(input, context);
		}
	...
	public static String getID(Modify modify)
		...
		String id = "";
		//会员信息删除_联系方式
		ContactInfo[] contactInfo =  modify.getLCCUserInfo().getContactInfoArray();
		 if(null != contactInfo && contactInfo.length>0){
			 id = contactInfo[0].getID();
